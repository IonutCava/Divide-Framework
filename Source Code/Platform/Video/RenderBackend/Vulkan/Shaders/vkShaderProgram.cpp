...
namespace Divide {
    ...
    bool vkShader::load(const ShaderProgram::LoadData& data) {
        ...
        if (data._type != ShaderType::COUNT) {
            ...
            // Build reflection data and descriptor sets and whatnot
            {
                SpvReflectShaderModule module;
                SpvReflectResult result = spvReflectCreateShaderModule(createInfo.codeSize, createInfo.pCode, &module);
                if (result != SPV_REFLECT_RESULT_SUCCESS) {
                    Console::errorfn(Locale::Get(_ID("ERROR_SHADER_REFLECTION_SPIRV_FAILED")), name().c_str());
                    return false;
                }
                ...
                vector<vkDescriptorSetLayoutData> set_layouts(sets.size(), vkDescriptorSetLayoutData{});
                for (size_t i_set = 0; i_set < sets.size(); ++i_set) {
                    const SpvReflectDescriptorSet& refl_set = *(sets[i_set]);
                    vkDescriptorSetLayoutData& layout = set_layouts[i_set];
