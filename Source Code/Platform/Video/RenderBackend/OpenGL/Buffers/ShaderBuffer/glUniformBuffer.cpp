    ...
    : ShaderBuffer(context, descriptor)
{
    ...
    const size_t targetElementSize = GLUtil::GLMemory::GetAlignmentCorrected(_params._elementSize, AlignmentRequirement(_usage));
...
void glUniformBuffer::clearBytes(ptrdiff_t offsetInBytes, const ptrdiff_t rangeInBytes) {
    if (rangeInBytes > 0) {
        ...
        DIVIDE_ASSERT(offsetInBytes == GLUtil::GLMemory::GetAlignmentCorrected((offsetInBytes), AlignmentRequirement(_usage)));
        ...
        GL_API::RegisterBufferLock(
            {
                ...
                to_size(GLUtil::GLMemory::GetAlignmentCorrected(rangeInBytes, AlignmentRequirement(_usage)))
            },
            lockType);
...
bool glUniformBuffer::bindByteRange(const U8 bindIndex, ptrdiff_t offsetInBytes, const ptrdiff_t rangeInBytes) {
    if (rangeInBytes > 0) {
        ...
        if (bindIndex == to_base(ShaderBufferLocation::CMD_BUFFER)) {
            ...
            const size_t range = GLUtil::GLMemory::GetAlignmentCorrected((offset == 0u && to_size(rangeInBytes) == bufferImpl()->memoryBlock()._size) ? 0u : rangeInBytes, AlignmentRequirement(_usage));
