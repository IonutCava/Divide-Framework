...
void UpdateTransformContainer(const Bone_uptr& node, BoneTransforms& containerOut)
{
    if ( node != nullptr)
    {
        if ( node->boneID() != Bone::INVALID_BONE_IDX)
        {
            containerOut[node->boneID()] = node->_offsetMatrix * GetBoneGlobalTransform(*node);
        }
...
U8 SceneAnimator::boneIndexByNameHash(const U64 nameHash) const
{
    ...
    return bone != nullptr ? bone->boneID() : Bone::INVALID_BONE_IDX;
}
