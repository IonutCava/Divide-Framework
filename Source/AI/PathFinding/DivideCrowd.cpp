...
void DivideDtCrowd::getActiveAgents( vector<dtCrowdAgent*>& agentsOut ) const
{
    agentsOut.resize(getMaxNbAgents());

    const I32 size = _crowd->getActiveAgents( agentsOut.data(), getMaxNbAgents());
    agentsOut.resize(size);
}
